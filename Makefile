.PHONY: help tree check-structure check-infra check-project-code check-dbt floe-manifest floe-manifest-upload dbt-parse project-code-image project-code-load local-cluster local-destroy-cluster local-up local-down local-status local-forward local-prefetch
...
help:
	...
	@printf '%s\n' '  make local-prefetch    Pre-pull heavy images (OpenSearch, OM ingestion) into kind to avoid Helm timeouts'
	@printf '%s\n' '  make local-up         Terraform-apply SeaweedFS + Polaris + Trino + OpenMetadata + Dagster'
	...
	@printf '%s\n' '  make local-forward    Port-forward all services to localhost (Dagster :3000, OpenMetadata :8585, Trino :8080, Polaris :8181, S3 :9000)'
...
local-forward:
	...
	om_pid=$$!; \
	trap 'kill $$seaweedfs_pid $$polaris_pid $$trino_pid $$dagster_pid $$om_pid 2>/dev/null || true' INT TERM EXIT; \
	wait
