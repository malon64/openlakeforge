...
| Floe profile variables do not currently render storage definitions. | Product Floe configs carry local S3 storage defaults because `floe manifest generate` does not substitute profile `variables` into config-level storage definitions; AWS generation renders a temporary provider config before invoking Floe rather than patching the manifest afterward. | Remove the temporary config renderer once Floe supports this path or exposes a first-class override mechanism. | Upstream issue filed as [malon64/floe#424](https://github.com/malon64/floe/issues/424). | No |
