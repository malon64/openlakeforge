...
| OpenLineage remains partially disabled in OpenMetadata. | Native OpenMetadata ingestion now records trustworthy dbt-trino Silver→Gold lineage, but Floe cannot yet emit Bronze→Silver lineage to OpenMetadata's endpoint. | Keep the dbt-trino entity-resolution test and track Floe's configurable endpoint; do not add a proxy or local workaround. | Local Trino-only lineage validation passed. Defer Bronze→Silver capture until [malon64/floe#450](https://github.com/malon64/floe/issues/450) ships, then validate it on each provider. | Pending |
