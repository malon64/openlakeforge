...
| Floe manifest replay omits dataset context from OpenLineage events. | Floe 0.6.10 securely authenticates to OpenMetadata and emits lifecycle events, but manifest replay constructs its lineage observer without the manifest entities. Entity events therefore lack the dataset context needed for Bronze→Silver edges and column lineage. | Keep direct authenticated emission enabled without a proxy; consume an upstream replay fix that reconstructs observer entities from the manifest. | Endpoint and runtime credential support shipped in [malon64/floe#451](https://github.com/malon64/floe/pull/451) and [malon64/floe#453](https://github.com/malon64/floe/pull/453). Capture and validate dataset lineage after the replay gap is fixed. | No |
