...
| --- | --- | --- | --- | --- |
| Dynamic artifact revision drift is still possible outside the deploy flow. | Resolved: the local, Azure, and AWS artifact deploy flows compute a content hash over the generated Floe artifact set, bake it into the `project-code` image as an OCI label and env var, stamp it onto every uploaded object plus a `floe/manifests/REVISION` sidecar, and patch it into the Dagster env in the same rollout as the image. Dagster fails fast with `ArtifactRevisionError` when the image label, runtime expectation, and fetched manifest hash disagree, and never falls back to a stale local manifest. | Keep the invariant documented in `docs/architecture/local-stack-contracts.md` ("Artifact Revision Contract"), keep the interrupted-deploy negative test in `tools/olf/tests/test_revision.py`, and keep the sidecar-versus-bucket assertion in the full e2e suite. | Local validation and the negative test pass; validate against live Azure and AWS deploys before relying on the cloud paths. | Yes |
