...
[src/](src/) — regenerate with `python3 src/spec_chartN.py`, then run
`python3 src/lint_charts.py`, which fails if any text escapes its box or the canvas
(the composer has no layout engine, so this is the guard against a label silently
spilling). Chart 3 is a Mermaid
