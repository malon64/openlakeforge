...
| Orchestration | Dagster Helm release, domain code locations, and project-code image loaded into kind | Same Dagster contract, registry-published images and cloud artifact storage |
| Artifacts | `artifacts.local_kind_and_s3` with `openlakeforge-ops` | `artifacts.ecr_and_s3` or provider object storage |
...
| Observability | `observability.object_log_archive` | Loki/Grafana, managed metrics, or provider log adapters later |
