...
```bash
uv run --project tools/olf olf deploy --provider local   # bring up the local kind stack
uv run --project tools/olf olf destroy --provider local  # bring down the local stack
uv run --project tools/olf olf e2e run --env local        # full runtime verification
```
