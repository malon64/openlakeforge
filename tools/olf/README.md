...
| `olf inventory terraform-external` | Render the typed, provider-neutral domain inventory for Terraform's external data source. Every environment root invokes this directly (see [ADR 0021](../../docs/adr/0021-terraform-reads-domain-inventory-via-external-provider.md)); `terraform plan`/`apply` therefore requires `uv` on `PATH`. |
