...
def artifacts_upload_manifests(
    ...
    # revision without Dagster's code server ever being told. It must NOT be
    # rolled from here: the running image's OPENLAKEFORGE_FLOE_MANIFEST_REVISION_BUILT
    # is an OCI-label-derived env var baked in at image build time (see
    # libs/product_dagster.py's _validate_artifact_revision), so patching only
    # the OPENLAKEFORGE_FLOE_MANIFEST_REVISION expectation onto the *same*
    # image guarantees the built-vs-expected mismatch _validate_artifact_revision
    # itself enforces -- every new pod would immediately fail closed, turning a
    # successful manifest upload into a self-inflicted Dagster outage. A
    # revision-changing upload can only be picked up safely by a full deploy
    # that rebuilds the image at the new revision. Warn instead of touching
    # the deployment.
    ...
    try:
        ...
        if current_revision and current_revision != "manual" and current_revision != full_bucket_manifest.revision:
            ...
            log.warn(
                f"Artifact revision contract is active ({current_revision!r} deployed) and this upload "
                f"advanced the bucket to revision {full_bucket_manifest.revision!r}, but the running "
                "project-code image was built for the old revision. Not rolling Dagster: patching only the "
                "revision expectation onto the same image would make every new pod fail the artifact "
                "revision check at startup. Run a full artifact deploy (which rebuilds the image at the "
                "new revision) to bring Dagster back in sync."
            )
        ...
        log.warn(
            ...
            "revision. Run a full artifact deploy to be certain."
        )
